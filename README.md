...
- 굵은 글씨체로 나오는 프로젝트가 시작 프로젝트
    1. 시작 프로젝트로 변경할 프로젝트 -> 마우스오른쪽 -> 시작프로젝트로 설정
    2. 전체 솔루션 -> 속성 -> 속성페이지
...
4. PyQt Designer와 거의 유사
    ...
    - StartPosition > CenterScreen 으로 변경
    - MaximizeBox > False
...
8. 중간마다 Ctrl + S, Ctrl + Shift + S(모두 저장) 눌러 저장 습관
...
10. 새로 만든 컨트롤을 더블클릭
    - 이벤트 메서드가 자동 생성
    - 최초의 아무런 이벤트가 없을 때 기본 이벤트 메서드 생성

11. 오류 발생 시
    - 폼 디자인의 컨트롤 이벤트와 로직 코드 상의 이벤트 처리 메서드 생성 상 불일치로 발생
    - 이벤트 이름은 생성되고 이벤트 정의 메소드는 생성이 안되서
    <img src ="./image/cs0002.png" witdth="600">

    - FrmMain.Designer.cs 파일 오픈 후 오류난 이벤트 이름을 삭제
...
- `TextBox` : 텍스트 입력을 위한 컨트롤
    ...
    - PasswordChar : 들어가는 텍스트를 암호처럼 숨길때 사용. ㅁ한자 키로 특수문자 ● 를 사용
    - Multiline : 여러줄 사용여부
...
- `ComboBox` : 여러개 중 아이템을 선택하는 컨트롤
    ...
    - **SelectedIndexChanged** : 선택한 아이템 순번이바뀔 때 발생하는 이벤트
    - **SelectedValueCahnged** : 선택한 아이템 값이 바뀔 때 발생하는 이벤트
...
- `MaskedTextBox` : 사용자에게 주어진 형태로만 입력을 하도록 강제하는 텍스트입력컨트롤
    - 텍스트 박스와 동일하게 Txt~ 로 시작
    - Button 컨트롤과 속성 동일
    - Mask : 사용자에게 입력제약할 표맷
- `GroupBox` : 관련있는 컨트롤들을 묶어주는 컨트롤
    - 이름을 거의 바꾸지 않음(바꾸는 경우 드뭄)
    - Button 컨트롤과 속성 동일
    - Text 속성만 사용
...
- `PictureBox` : 이미지를 표현해주는 컨트롤
    - Pic~ 시작, Pbx~
    - BorderStyle : F1xedSingle로 실선
    - BackColor : 폼색과 다르게 해서 영역표시
    - Image : 표시할 이미지 선택
    - Enabled : 사용여부
    - Image : 표시할 이미지 선택
    - Size : 크기
    - SizeMode : 들어가는 이미지의 크기 종류 (Nomal,Zoom 등등)
    - 들어갈 이미지의 속성
        - 
...
- Debug - 디버깅시 필요한 기능 담은 클래스
    - Debug.WriteLine() : 디버그 > 창 > `출력`에디버깅 로그 출력
    - 디버깅 시작으로 실행 시만 출력
- 프로젝트 속성
    - 출력 유형 : Windows 애플리케이션 > 콘솔애플리케이션으로 변경
...
- 기본 문법 - C++, Java와 거의 동일. C++와의 차이점
    ...
    4. 분기문,반복문 [소스](./day02/Day02Study/SyntaxWinApp03/FrmMain.cs)
        ...
        - foreach는 컬렉션에서...
        - while 문
        <img src ="./image/cs0003.png" witdth="600">
...
- 기본 문법
    ...
    <img src ="./image/cs0004.png" witdth="600">
    ...
    3. 접근제어자
        ...
        - `internal` : 동일한 네임스페이스 에서 public 기능 ex(SyntaxWinApp01)
...
- 고급 문법
    ...
    6. 비동기, 스레드 `중요` - [소스](./day05/Day05Study/SyntaxWinApp03/FrmMain.cs)
        - UI 프로그램에 `응답없음` 발생
...
- `TreeView` : 탐색기 하드디스크, 폴더 나타내는 부분과 동일한 컨트롤
    ...
    - AfterSelcet, AfterExpand, AfterCollapse 이벤트를 주로 사용
...
- `ListView` : 탐색기 오른쪽 목록을 표시하는 컨트롤
    ...
    - SelectedIndexChanged : 아이템 선택이 변경될 때 이벤트 발샘

- `MonthCalendat` : 스케줄 등록을 위한 컨트롤
    - Cal~ 로 이름 사용
...
- `MenuStrip` : 프로그램 메뉴관리
    - Mnu~ 이름시작. 사용하는 메뉴는 반드시 이름을 변경할것 (예 :끝내기XMenuStrip~~ <-이런식으로 이름이 나와보기안좋음)
    - 메뉴 이름 작성시 (&X) 와 같이 사용하면 영문자아래 밑줄표시
    - 빈공간 (-) 사용시 한줄 그어줌
    - Click : 해당 메뉴 클릭 이벤트
...
- 데이터베이스 데이터 바인딩
    ...
    3. 비하인드 코드에서 MySQL 소스 추가
        ...
        - 트랙잰셕객체 : 옵션. 트랜잭션(INSERT, UPDATE, DELETE 쿼리) 처리시 필요
...
- 사용법 [MahApps.Metro - Quick Start](https://mahapps.com/docs/guides/quick-start)
1. NuGet 패키지 관리 >
   - MahApps.Metro 설치
...
6. MainWindow.xaml
    - Window -> mah.MetroWindow로 변경
        - <mah:MetroWindow x:Class="윈폼타이틀.MainWindow" 변경
